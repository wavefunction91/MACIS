...
int main(int argc, char** argv) {
  ...
  size_t norb_chk; double E_core_chk;
  ...
  double max_diff = 0;
  for(auto i = 0; i < norb2; ++i) max_diff = std::max(max_diff, std::abs(T[i] - T_chk[i]));
  std::cout << "T_CHECK    = " << max_diff << std::endl;
  max_diff = 0;
  for(auto i = 0; i < norb2; ++i) max_diff = std::max(max_diff, std::abs(V[i] - V_chk[i]));
