...
template <typename RandomIt, typename ValueType, class OrderCompare, class EqualCompare>
...
Integral total_gather_and_exclusive_scan( Integral val, 
  std::vector<Integral>& gather, std::vector<Integral>& scan, MPI_Comm comm ) {
...
typename RandomIt::value_type dist_quickselect( RandomIt begin, RandomIt end,
  int k, MPI_Comm comm, OrderCompare ord_comp, EqualCompare eq_comp ) {
