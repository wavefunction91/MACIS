...
namespace macis
{
   ...
   *             Hamiltonian.
   * @param [in] const std::vector<std::complex<double> > &freqs: Frequency grid over which to evaluate the 
   *             resolvent.
   * @param [inout] std::vector<std::complex<double> > &gf: On successful exit, green's function along the
   *                frequency grid.
  ...
  void GF_Diag(
    ...
    if( saveABtofile )
    {
      ...
      for(int i = 0; i <= kry_size; i++) ofile << std::scientific << alphas[i] << std::endl;
      ofile.close();
      ofile.open(fpref + "betas.dat", std::ios::out);
      ofile.precision( dbl::max_digits10 );
      for(int i = 0; i <= kry_size; i++) ofile << std::scientific << betas[i] << std::endl;
      ofile.close();
    }
   ...
   * @param [in] const std::vector<double> &occs: Occupation numbers of all orbitals. Used to define
   *             active spaces.
   * @param [in] const GFSettings &settings: Includes input parameters, in particular:
    ...
    const GFSettings &settings)
  {
    ...
    std::cout << "COMPUTING GF SPACE IN *" << (is_part? "PARTICLE" : "HOLE") << "* SECTOR FOR ORBITAL " << orb << ", WITH SPIN *" << (sp_up ? "UP" : "DOWN") << "*" << std::endl;
    ...
    for(int iii = 0; iii < as_orbs.size(); iii++) std::cout << as_orbs[iii] << ", ";
    std::cout << "]" << std::endl;
    ...
    std::cout << "--BEFORE ADDING SINGLE-DOUBLES, nterms = " << cgf+1 << std::endl;
    ...
    std::cout << "--AFTER CUTTING BY: " << GFseedThreshold << ", GF-SPACE WITH GF SEED: " << GFseed << ", WE HAVE STATES: " << cgf+1 << std::endl;
   ...
   * @param [in] const std::vector<int> &GF_orbs: Orbital indices for which to compute the GF. 
   ...
   * @param [out] std::vector<int>& todelete: On return, contains a list of the orbitals for which the
     ...
     double zero_thresh = 1.E-7 )
  {
    ...
    std::vector<std::vector<double>> wfns(GF_orbs.size(), std::vector<double>(nterms,0.));
   ...
   * @param [in] const std::vector<int> &GF_orbs: Orbital indices for the Green's function, as requested
   ...
   * @param [in] const std::vector<int> &todelete: List of deleted orbital indices in the case just described.
   ...
   * @date 02/02/2022
   */
  void write_GF( const std::vector<std::vector<std::vector<std::complex<double> > > > &GF, const std::vector<std::complex<double> > &ws, const std::vector<int> &GF_orbs, const std::vector<int> &todelete, const bool is_part );
   ...
   * @param [in] const std::vector<std::complex<double> > &ws: Frequency grid over which to compute the Green's function.
   * @param [in] const std::vector<double> &occs: Occupation numbers for each orbital.
   * @param [in] const GFSettings &settings: Structure with various parameters for Green's function calculation.
   *
   * @author Carlos Mejuto Zaera
   * @date 01/02/2022
   */
    ...
    const GFSettings &settings)
  {
    ...
    size_t nterms = gf_dets.size();
    std::cout << "---> FINAL ADD BASIS HAS " << nterms << " ELEMENTS" << std::endl;
  
    loop1 = time(NULL);
    loop1C = Clock::now();
    auto hamil = make_dist_csr_hamiltonian<index_t>( MPI_COMM_WORLD, gf_dets.begin(), 
                                                           gf_dets.end(), Hgen, h_el_tol );
    loop2 = time(NULL);
    loop2C = Clock::now();
    std::cout << std::setprecision(3)<<"Building " << (is_part ? "*PARTICLE*" : "*HOLE*") << " Hamiltonian: " << double(std::chrono::duration_cast<std::chrono::milliseconds>(loop2C - loop1C).count())/1000 << std::endl;
    ...
    //PREPARE THE WAVEFUNCTIONS FOR THE BAND LANCZOS
    std::vector<int> todelete;
    std::vector<std::vector<double>> wfns = BuildWfn4Lanczos<nbits, index_t>( wfn0, GF_orbs_comp, is_up_comp, 
    ...
    // //ACTUALLY COMPUTE THE GF!
     ...
     std::cout << std::setprecision(3)<<"Computing GF with " << (use_bandLan ? " *Band Lanczos*" : "*Regular Lanczos*") << double(std::chrono::duration_cast<std::chrono::milliseconds>(GF_loop2C - GF_loop1C).count())/1000 << std::endl;
