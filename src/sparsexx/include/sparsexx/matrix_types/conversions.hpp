...
void csr_to_csc( size_t M, size_t N, 
  const index_t* Ap, const index_t* Ai, const T* Az,
  index_t*       Bp, index_t*       Bi, T*       Bz, 
  size_t indexing ) {
...
template <typename T, typename index_t, typename Alloc>
csc_matrix<T,index_t,Alloc>::csc_matrix( const coo_matrix<T,index_t,Alloc>& other ) :
  csc_matrix( other.m(), other.n(), other.nnz(), other.indexing() ) {

  if( not other.is_sorted_by_col_index() ) {
    throw 
      std::runtime_error("COO -> CSC Conversion Requires COO To Be Column Sorted");
  }
...
template <typename T, typename index_t, typename Alloc>
csr_matrix<T,index_t,Alloc>::csr_matrix( const csc_matrix<T,index_t,Alloc>& other ) :
  csr_matrix( other.m(), other.n(), other.nnz(), other.indexing() ) {

  detail::csr_to_csc( n_, m_, 
    other.colptr().data(), other.rowind().data(), other.nzval().data(),
    rowptr_.data(), colind_.data(), nzval_.data(), indexing_ );
