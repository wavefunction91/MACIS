...
int main( int argc, char** argv ) {
  ...
  {
  auto ierr = LanTrbounds( 200, 2000, 1e-8, vinit.data(), 1, &lmin, &lmax, fstats );
  ...
  if( ecount < 0 or ecount > N )
    throw std::runtime_error("ECOUNT IS INVALID");
  ...
  {
    auto ierr = spslicer( sli.data(), mu.data(), mdeg, xintv.data(), nsli,
      npts );
  ...
  std::cout << "EVSL will try to produce " << nev_sli << " eigenvalues per slice" << std::endl;
  ...
  for( auto i = 0; i < nsli; ++i ) {
    ...
    std::cout << std::endl;
    std::cout << "  * Processing interval [" << a << ", " << b << "]" << std::endl;
    ...
    {
      auto ierr = ChebLanTr( mlan, nev_sli, xintv.data(), max_lan_it,
        tol, vinit.data(), &pol, &nev_out, &lam_slice, &Y, &res,
        fstats);
    ...
    std::sort( ind.begin(), ind.end(), [&](auto i, auto j) { return lam_slice[i] < lam_slice[j]; } );

    std::cout << "  * Found " << nev_out << " eigenvalues in slice" << std::endl;
    for(auto k = 0; k < nev_out; ++k) {
      std::cout << "    " << k << ": " << lam_slice[ind[k]] << ", " << res[ind[k]] << std::endl;
    }
