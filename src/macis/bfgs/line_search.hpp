...
#define USE_CARLOS_VAL
    ...
    logger->debug( 
      "tau = {:.4f}, c1 = {:.4f}, c2 = {:4f}, t_armijo = {:10.7e}, t_swolfe = {:10.7e}", 
    ...
    if(fx - fx0 > 0 or step < 1e-6 ) throw std::runtime_error("Line Search Failed");
...
void discrete_line_search(
  ...
  for(size_t ig = 1; ig < ngrid; ++ig) {
    ...
    x = x0; Functor::axpy(temp_step, p, x);
    fx = op.eval(x);
  ...
  logger->debug("MinVal at S = {:10.6e} F(X) = {:15.12f}, dF = {:20.12e}",
    step, fx, (fx - fx0));
}
