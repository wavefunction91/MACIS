...
namespace macis
{
  void write_GF( const std::vector<std::vector<std::vector<std::complex<double> > > > &GF, const std::vector<std::complex<double> > &ws, const std::vector<int> &GF_orbs, const std::vector<int> &todelete, const bool is_part )
  {
     ...
     if(GF_orbs.size() > 1)
     {
       ...
       for(int iii = 0; iii < nfreqs; iii++){ 
         ...
         for(int jjj = 0; jjj < GF[iii].size(); jjj++){
           for(int lll = 0; lll < GF[iii][jjj].size(); lll++)ofile << std::scientific << real(GF[iii][jjj][lll]) << " " << imag(GF[iii][jjj][lll]) << " ";
         }
         ofile << std::endl;
       }
       ofile.close();
     ...
     else
     {
       ...
       std::ofstream ofile( fname );
       ofile.precision(dbl::max_digits10);
       for(int iii = 0; iii < nfreqs; iii++) ofile << std::scientific << real(ws[iii]) << " " << imag(ws[iii]) << " " << real(GF[iii][0][0]) << " " << imag(GF[iii][0][0]) << std::endl;
       ofile.close();
     }
  }
