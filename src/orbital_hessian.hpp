...
inline void approx_diag_hessian_2(NumInactive ni, NumActive na, NumVirtual nv,
  const double* Fi, size_t LDFi, const double* Fa, size_t LDFa,
  const double* Q, size_t LDQ, const double* A1RDM, size_t LDD, 
  const double* F, size_t LDF, double* H_lin) {

  auto [H_vi, H_va, H_ai] = split_linear_orb_rot(ni, na, nv, H_lin);
  approx_diag_hessian_2(ni, na, nv, Fi, LDFi, Fa, LDFa, Q, LDQ, A1RDM, LDD, 
    F, LDF, H_vi, H_va, H_ai);

}

template <typename... Args>
void approx_diag_hessian_2(NumOrbital norb, NumInactive ninact, NumActive nact,
  NumVirtual nvirt, const double* T, size_t LDT, const double* V, size_t LDV,
  const double* A1RDM, size_t LDD1, const double* A2RDM, size_t LDD2,
  Args&&... args) {
  ...
  // Compute approximate diagonal hessian
  approx_diag_hessian_2(ninact, nact, nvirt, Fi.data(), no, Fa.data(), no,
    Q.data(), na, A1RDM, LDD1, F.data(), no, std::forward<Args>(args)... );
}
